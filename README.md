...
La landing page contiene versión en **español** e **inglés** dentro del mismo proyecto estático. El cambio de idioma se realiza desde el botón ubicado en la barra superior:

- `English`: cambia la interfaz al idioma inglés.
- `Español`: regresa la interfaz al idioma español.

El sitio conserva el idioma elegido mediante `localStorage`, por lo que el usuario mantiene su preferencia al recargar la página. La internacionalización se gestiona desde `app.js` mediante:

- diccionario `STATIC_TRANSLATIONS` para textos fijos de la interfaz;
- campo `translations.en` dentro de `projects.json` para los textos del portafolio;
- renderizado dinámico de categorías, filtros, tarjetas, modales, botones y mensajes de estado según idioma activo.
...
- **Versión integral:** v16
- **Sitio objetivo:** `https://skepsis-apps.github.io/landing_page/`
...
La sección de contacto orienta al usuario a iniciar conversación por WhatsApp o correo electrónico:
...
Esta versión incorpora soporte bilingüe completo:

- interfaz corporativa en español e inglés;
- botón de idioma en la barra de navegación con banderas `🇬🇧` y `🇪🇸`;
- traducción de secciones principales, filtros, botones, mensajes, modales y textos del portafolio;
- traducciones en `projects.json` mediante `translations.en`;
- conservación del idioma seleccionado con `localStorage`.


---

## Corrección v20 — botón de idioma en GitHub Pages

Se agregó control de caché mediante parámetros de versión en `index.html`:
...
También se reforzó el enlace del evento del botón de idioma en `app.js` para mejorar la compatibilidad en GitHub Pages.


## Actualización v21: arquitecturas bilingües

Esta versión incorpora soporte visual bilingüe para la sección de arquitectura de los siete portafolios.  
Cada proyecto mantiene su arquitectura en español y agrega una versión equivalente en inglés mediante el campo:

```json
"translations": {
  "en": {
    "architectureImage": "assets/architectures/<archivo>-en.svg"
  }
}
```

Además, en la arquitectura de OpenLogi RFID IoT ML se corrigió el texto:
