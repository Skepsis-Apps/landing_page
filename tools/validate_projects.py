#!/usr/bin/env python3
"""
Validador estructurado para projects.json bilingüe.

Propósito:
    Validar la estructura mínima y las traducciones opcionales del archivo de proyectos usado por la landing
    de GitHub Pages.
...
def validate_optional_translation(project: dict[str, Any], index: int) -> None:
    """
    Valida la estructura opcional de traducciones del proyecto.
    ...
    Raises:
        ValueError: Si translations.en existe pero no respeta la estructura esperada.
    """
    ...
    if not isinstance(translations, dict):
        raise ValueError(f"Proyecto {index}: translations debe ser un objeto.")
    ...
    if not isinstance(english, dict):
        raise ValueError(f"Proyecto {index}: translations.en debe ser un objeto.")

    required_text_fields: list[str] = [
        "category",
        "summary",
        "description",
        "purpose",
    ...
    for field in required_text_fields:
        ...
        if not isinstance(value, str) or value.strip() == "":
            raise ValueError(
                f"Proyecto {index}: translations.en.{field} debe ser texto no vacío."
            )

    translated_features = english.get("features")
    if not isinstance(translated_features, list) or len(translated_features) == 0:
        raise ValueError(f"Proyecto {index}: translations.en.features debe ser lista no vacía.")

    for item_index, item in enumerate(translated_features):
        if not isinstance(item, str) or item.strip() == "":
            raise ValueError(
                f"Proyecto {index}: translations.en.features[{item_index}] debe ser texto no vacío."
            )
...
def validate_projects(projects: list[dict[str, Any]]) -> bool:
    ...
    for index, project in enumerate(projects):
        ...
        validate_optional_translation(project, index)
